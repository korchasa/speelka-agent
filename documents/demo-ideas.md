````
Твоя задача - следуя по свои инструкциям, ты должен придумать пять впечатляющих и очень удобных применений в повседневной жизни для проекта Speelka Agent, с использованием MCP(Model Context Protocol) серверов из списка ниже. Speelka Agent это проект, который дает LLM возможность запускать любые инструменты из разных MCP-серверов. Учти, что Speelka Agent не может быть запущен постоянно и запускается от какой-то просьбы пользователя в чате.
