...
func ConvertToolToLLM(tool mcp.Tool) (llms.Tool, error) {
    ...
    if len(tool.RawInputSchema) > 0 {
        ...
        // Случай когда InputSchema полностью пустая или не инициализирована
        // Создаем пустой объект в качестве параметров
        parametersMap = map[string]interface{}{
            "type":       "object",
            "properties": map[string]interface{}{},
        }
    } else {
        // Проверяем, есть ли свойства в схеме ввода
        if tool.InputSchema.Properties == nil {
            // Если свойств нет, это инструмент без аргументов
            // Создаем пустой объект в качестве параметров
            parametersMap = map[string]interface{}{
                "type":       "object",
                "properties": map[string]interface{}{},
            ...
            // Преобразуем InputSchema в мапу
